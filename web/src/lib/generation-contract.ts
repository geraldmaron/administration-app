...
function normalizeRegionIds(values?: string[]): string[] {
...
function inferDefaultSourceKind(mode?: GenerationMode): ScenarioSourceKind {
  return mode === 'news' ? 'news' : 'evergreen';
}

function inferScopeKey(
  scopeTier: ScenarioScopeTier,
...
export function normalizeGenerationScope(input: GenerationScopeInput): GenerationScopeNormalizationResult {
  ...
  const sourceKind: ScenarioSourceKind = validSourceKinds.includes(rawSourceKind) ? rawSourceKind : inferDefaultSourceKind(input.mode);
