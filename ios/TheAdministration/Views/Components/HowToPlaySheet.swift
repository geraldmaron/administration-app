...
private let guideSections: [GuideSection] = [
    ...
    GuideSection(
        ...
        body: "Instead of choosing from the three authored options, you can write your own policy response in free text. This is Trust Your Gut.\n\nYou have a limited number of uses per run — between 3 and 6, depending on your cabinet size (base 3, plus one for every 4 filled cabinet roles). Each use carries a small upfront cost to approval, foreign relations, and the economy — improvisation has a price. There is no per-turn cooldown; your uses are drawn from a total pool for the entire run.\n\nTrust Your Gut lets you govern outside the authored playbook. Your input is processed by the game's AI layer and resolved into real consequences, subject to the same realism rules as everything else. It is powerful, but scarce — use it when none of the provided options fit your strategy."
    ),
    GuideSection(
        id: "metrics",
        label: "INDICATORS",
        title: "The Metric System",
        body: "Your nation is tracked across 27 metrics, organized in four layers:\n\nCore metrics (18) — Economy, Public Order, Healthcare, Education, Infrastructure, Environment, Foreign Relations, Military, Liberty, Equality, Employment, Innovation, Trade, Energy, Housing, Democracy, Sovereignty, Immigration. Range 0–100. Higher is better.\n\nInverse metrics (4) — Corruption, Inflation, Crime, Bureaucracy. Range 0–100. Lower is better. These are easy to let drift upward unnoticed.\n\nBudget (1) — Ranges −100 to +100. Positive is surplus; negative is deficit.\n\nHidden metrics (3) — Unrest, Economic Bubble, and Foreign Influence begin hidden and only appear on your dashboard once they breach a reveal threshold. By then, they are already a problem. Lower is better.\n\nApproval is derived — it is not a metric you can directly control. It is recalculated every turn from a weighted formula across your core indicators, with economy, inflation, employment, and healthcare carrying the most weight.\n\nMetrics are not independent. Economic decline spreads to employment, housing, unrest, and public order. High corruption erodes liberty and foreign relations. Neglect long enough in the wrong areas, and cascade effects become very hard to reverse."
    ),
    GuideSection(
        id: "cabinet",
        label: "PERSONNEL",
    ...
    GuideSection(
        ...
        label: "TIMELINE",
        title: "Game Length",
        body: "At setup, you choose a mode: Short, Medium, or Long. This sets the approximate length of your term.\n\nShort: roughly 27–33 turns\nMedium: roughly 54–66 turns\nLong: roughly 108–132 turns\n\nThe exact turn count has some variance — it is fixed at the start of the run, not at a round number. Scenario phases (Early, Mid, Late, Endgame) shift as you move through your term, changing the types of scenarios that surface.\n\nYour run can end before the final turn. With a neutral legislature, your administration collapses if approval falls below 15, or if three or more of the four core stability metrics — Approval, Economy, Foreign Relations, and Public Order — simultaneously fall below 20. A hostile legislature can act on weaker grounds; a loyal one gives you more runway. You do not have to reach the last turn to trigger the end-game review."
    ),
    GuideSection(
        id: "endgame",
        label: "LEGACY",
