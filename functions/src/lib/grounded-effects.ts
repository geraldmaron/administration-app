...
interface GroundedMapping {
  targetMetrics: string[];      // Primary metrics affected
  perUnitDelta: number;         // Effect magnitude per 1 unit (1%, 1000 people, etc.)
  maxReference: number;         // Cap on reasonable references
  unit: string;                 // Unit label for logging
  isInverse?: boolean;          // If true, positive narrative = negative game effect
}
 ...
 */
const GROUNDED_MAPPINGS: Record<string, GroundedMapping> = {
  // Economic references
  gdp: {
...
const NARRATIVE_PATTERNS: { pattern: RegExp; mapping: string; multiplier?: number }[] = [
  // Percentage patterns
...
function parseNumber(str: string): number {
  ...
  if (lower.includes('million') || lower.includes('m')) return num * 1_000_000;
  if (lower.includes('thousand') || lower.includes('k')) return num * 1_000;

  return num;
}

/**
 * Extract grounded references from narrative text
 */
export interface ExtractedReference {
  mapping: string;
  value: number;
  originalText: string;
  expectedEffects: { metricId: string; value: number }[];
}

export function extractNarrativeReferences(text: string): ExtractedReference[] {
...
/**
 * Validate that option effects match narrative references
 * Returns issues if effects don't align with described impacts
 */
export interface GroundingIssue {
  type: 'missing' | 'magnitude' | 'sign';
 ...
 */
export function suggestEffectCorrections(
  narrativeText: string,
  ...
  for (const ref of references) {
    for (const expected of ref.expectedEffects) {
      ...
      if (!existing) {
        ...
        if (corrected.length >= 4) continue;
        corrected.push({
 ...
 */
export function getEffectCalibrationPrompt(): string {
 ...
 */
const MONETARY_AMOUNT_PATTERNS: { pattern: RegExp; parseAmount: (match: RegExpExecArray) => number }[] = [
  // "$X trillion" or "X trillion dollars/euros/etc"
  {
    pattern: /\$?\s*(\d+(?:\.\d+)?)\s*trillion/gi,
    parseAmount: (m) => parseFloat(m[1]) * 1_000_000, // trillions → millions
  },
  // "$X billion" or "X billion dollars/euros/etc"
  {
    pattern: /\$?\s*(\d+(?:\.\d+)?)\s*billion/gi,
    parseAmount: (m) => parseFloat(m[1]) * 1_000, // billions → millions
  },
...
export interface MonetaryRealismIssue {
  type: 'unrealistic-amount' | 'unrealistic-theft';
 ...
 */
export function validateMonetaryRealism(
  text: string,
  gdpMillions: number
): MonetaryRealismIssue[] {
  if (!text || gdpMillions <= 0) return [];

  const issues: MonetaryRealismIssue[] = [];
  ...
  for (const { pattern, parseAmount } of MONETARY_AMOUNT_PATTERNS) {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (!Number.isFinite(amountMillions) || amountMillions <= 0) continue;
