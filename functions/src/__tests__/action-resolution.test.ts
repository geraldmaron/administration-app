...
function makeDiplomaticRequest(overrides: Partial<ActionResolutionRequest> = {}): ActionResolutionRequest {
    return {
        ...
        maxTurns: 30,
        phase: 'early',
        metrics: { metric_approval: 55, metric_economy: 60, metric_foreign_relations: 50 },
        relationship: 40,
...
function makeMilitaryRequest(overrides: Partial<ActionResolutionRequest> = {}): ActionResolutionRequest {
    return {
        ...
        countryId: 'us',
        countryName: 'United States',
        targetCountryName: 'Iran',
        turn: 15,
        maxTurns: 30,
        phase: 'mid',
        metrics: { metric_approval: 45, metric_economy: 50, metric_foreign_relations: 35 },
        relationship: -60,
        relationshipType: 'adversary',
        targetMilitaryStrength: 65,
        targetNuclearCapable: false,
        ...overrides,
    };
}

function makeTygRequest(overrides: Partial<ActionResolutionRequest> = {}): ActionResolutionRequest {
    return {
        actionCategory: 'trust_your_gut',
        actionType: 'freeform',
        freeFormCommand: 'Impose emergency economic controls on all imports',
        countryId: 'us',
        countryName: 'United States',
        turn: 10,
        maxTurns: 30,
        phase: 'mid',
        metrics: { metric_approval: 50, metric_economy: 45 },
        ...overrides,
    };
}

function makeRawResponse(overrides: Partial<ActionResolutionResponse> = {}): ActionResolutionResponse {
    return {
        headline: 'Trade Deal Signed with France',
        summary: 'A new bilateral trade agreement opens markets.',
        context: 'The agreement is expected to boost GDP by 0.3% over the next quarter.',
        metricDeltas: [
...
describe('clampMetricDeltas', () => {
    ...
    test('clamps deltas to bounds', () => {
        ...
        const result = clampMetricDeltas(deltas, { min: -10, max: 6 });
...
describe('validateRequest', () => {
    ...
    test('accepts valid military request', () => {
        expect(validateRequest(makeMilitaryRequest())).toBeNull();
    });

    test('accepts valid trust_your_gut request', () => {
        expect(validateRequest(makeTygRequest())).toBeNull();
    });

    test('rejects missing actionCategory', () => {
        const req = makeDiplomaticRequest({ actionCategory: '' as any });
        expect(validateRequest(req)).toContain('actionCategory');
    });

    test('rejects invalid actionCategory', () => {
        const req = makeDiplomaticRequest({ actionCategory: 'invalid' as any });
        expect(validateRequest(req)).toContain('Invalid actionCategory');
    });

    test('rejects diplomatic action without actionType', () => {
        const req = makeDiplomaticRequest({ actionType: undefined });
        expect(validateRequest(req)).toContain('actionType');
    });

    test('rejects trust_your_gut without freeFormCommand', () => {
        const req = makeTygRequest({ freeFormCommand: undefined });
        expect(validateRequest(req)).toContain('freeFormCommand');
    });

    test('rejects missing countryId', () => {
        const req = makeDiplomaticRequest({ countryId: '' });
        expect(validateRequest(req)).toContain('countryId');
    });

    test('rejects invalid turn', () => {
        const req = makeDiplomaticRequest({ turn: 0 });
        expect(validateRequest(req)).toContain('turn');
    });
});
