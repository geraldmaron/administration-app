...
async function isDuplicateOfExistingScenario(db, headline, bundle) {
    ...
    if ((0, semantic_dedup_1.isSemanticDedupEnabled)()) {
        try {
            const embedding = await (0, semantic_dedup_1.generateEmbedding)(headline);
            if (embedding.length > 0) {
                const similar = await (0, semantic_dedup_1.findSimilarByEmbedding)(embedding, bundle);
                if (similar) {
                    logger.info(`[NewsIngest] Semantic duplicate detected: "${headline}" ≈ ${similar.id} (similarity: ${similar.similarity.toFixed(3)})`);
                    return true;
                }
            }
        }
    ...
    try {
        ...
        for (const classified of deduped) {
            ...
            try {
                ...
                const genResult = await (0, scenario_engine_1.generateScenarios)(Object.assign({ mode: 'news', bundle, count: 1, newsContext: [newsItem], distributionConfig: { mode: 'fixed', loopLength: 1 } }, generationScope));
