...
exports.CONCEPT_TO_TOKEN_MAP = exports.CANONICAL_ROLE_IDS = exports.ROLE_ALIAS_MAP = exports.TOKEN_ALIAS_MAP = exports.ARTICLE_FORM_TOKEN_NAMES = exports.ALL_TOKENS = exports.TOKEN_CATEGORIES = void 0;
exports.isValidToken = isValidToken;
exports.normalizeTokenAliases = normalizeTokenAliases;
exports.buildTokenWhitelistPromptSection = buildTokenWhitelistPromptSection;
// ---------------------------------------------------------------------------
// 1. TOKEN_CATEGORIES — Categorized token definitions
// ---------------------------------------------------------------------------
exports.TOKEN_CATEGORIES = {
    executive: [
        'leader_title',
        'vice_leader',
        'head_of_state_title',
...
// ---------------------------------------------------------------------------
exports.ALL_TOKENS = [
    ...exports.TOKEN_CATEGORIES.executive,
    ...exports.TOKEN_CATEGORIES.legislative,
    ...exports.TOKEN_CATEGORIES.judicial,
    ...exports.TOKEN_CATEGORIES.ministers,
    ...exports.TOKEN_CATEGORIES.security,
    ...exports.TOKEN_CATEGORIES.military,
    ...exports.TOKEN_CATEGORIES.economic,
    ...exports.TOKEN_CATEGORIES.local,
    ...exports.TOKEN_CATEGORIES.civil,
    ...exports.TOKEN_CATEGORIES.media,
    ...exports.TOKEN_CATEGORIES.relationships,
    ...exports.TOKEN_CATEGORIES.context,
    ...exports.TOKEN_CATEGORIES.party,
    ...exports.TOKEN_CATEGORIES.article_forms,
];
// ---------------------------------------------------------------------------
// 3. ARTICLE_FORM_TOKEN_NAMES — Set of bare token names that have the_* forms
// ---------------------------------------------------------------------------
exports.ARTICLE_FORM_TOKEN_NAMES = new Set(exports.TOKEN_CATEGORIES.article_forms
    .filter((t) => t.startsWith('the_'))
    .map((t) => t.slice(4)));
// ---------------------------------------------------------------------------
// 4. TOKEN_ALIAS_MAP — Hallucinated token name → valid token name
// ---------------------------------------------------------------------------
exports.TOKEN_ALIAS_MAP = {
...
// ---------------------------------------------------------------------------
function normalizeTokenAliases(text) {
    let result = text;
    for (const [bad, good] of Object.entries(exports.TOKEN_ALIAS_MAP)) {
        result = result.replace(new RegExp(`\\{the_${bad}\\}`, 'gi'), `{the_${good}}`);
        result = result.replace(new RegExp(`\\{${bad}\\}`, 'gi'), `{${good}}`);
    }
    const exactPlaceholderReplacements = [
    ...
    result = result.replace(/\{(?:the_)?metric_([a-z_]+)\}/gi, (_match, metricName) => {
...
// ---------------------------------------------------------------------------
function buildTokenWhitelistPromptSection() {
    const categoryNames = Object.keys(exports.TOKEN_CATEGORIES).filter(k => k !== 'article_forms');
    const categorizedList = categoryNames.map(cat => {
        const tokens = exports.TOKEN_CATEGORIES[cat];
        const label = cat.charAt(0).toUpperCase() + cat.slice(1).replace(/_/g, ' ');
        return `- **${label}**: ${tokens.map(t => `{${t}}`).join(', ')}`;
    }).join('\n');
    const articleFormsList = exports.TOKEN_CATEGORIES.article_forms.map(t => `{${t}}`).join(', ');
    ...
    const invalidTokenEntries = Object.entries(exports.TOKEN_ALIAS_MAP)
        .map(([bad, good]) => `- \`{${bad}}\` → use \`{${good}}\``)
        .join('\n');
    const canonicalRolesList = Array.from(exports.CANONICAL_ROLE_IDS).map(r => `\`${r}\``).join(', ');
    ...
    const invalidRoleRedirects = Object.entries(exports.ROLE_ALIAS_MAP)
        .map(([bad, good]) => `- \`${bad}\` does NOT exist — use \`${good}\``)
        .join('\n');
    const conceptTable = exports.CONCEPT_TO_TOKEN_MAP
