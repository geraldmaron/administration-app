...
exports.repairTextIntegrity = repairTextIntegrity;
exports.getAuditConfig = getAuditConfig;
exports.setAuditConfigForTests = setAuditConfigForTests;
...
exports.normalizeScenarioTextFields = normalizeScenarioTextFields;
...
function repairTextIntegrity(text) {
    if (!text)
        return text;
    return text
        .replace(REPEATED_WORD_PATTERN, '$1')
        .replace(/\b(a|an|the|to|for|with|from|into|onto|of|in|on|at|by)\s+([,.;:!?])/gi, '$2')
        .replace(DANGLING_ENDING_PATTERN, (_match, _word, punctuation) => punctuation !== null && punctuation !== void 0 ? punctuation : '')
        .replace(/([.!?]){2,}/g, '$1')
        .replace(/\s+([.!?,;:])/g, '$1')
        .replace(/\s{2,}/g, ' ')
        .trim();
...
let _config = null;
const RELATIONSHIP_TOKEN_NAMES = new Set([
    'ally',
    'adversary',
    'border_rival',
    'regional_rival',
...
function setAuditConfigForTests(config) {
    _config = config;
}
function getCountryGeopoliticalProfile(country) {
    var _a, _b;
    return (_b = (_a = country === null || country === void 0 ? void 0 : country.geopoliticalProfile) !== null && _a !== void 0 ? _a : country === null || country === void 0 ? void 0 : country.geopolitical) !== null && _b !== void 0 ? _b : {};
}
function getRelationshipEntries(value) {
    return Array.isArray(value) ? value : [];
}
function hasRelationshipOfType(entries, types) {
    return entries.some((entry) => {
        const entryType = typeof (entry === null || entry === void 0 ? void 0 : entry.type) === 'string' ? entry.type.toLowerCase() : '';
        return types.includes(entryType);
    });
}
function supportsRelationshipToken(country, tokenName) {
...
function collectRelationshipTokens(text) {
    ...
    for (const match of matches) {
        ...
        if (RELATIONSHIP_TOKEN_NAMES.has(baseName)) {
            found.add(baseName);
        }
    }
    return Array.from(found);
}
function formatRelationshipToken(tokenName) {
...
function auditScenario(scenario, bundleCategory, genMode = false) {
    ...
    const checkVagueness = (text, fieldName) => {
        if (!text)
            return;
    ...
    const isUniversal = ((_d = scenario.metadata) === null || _d === void 0 ? void 0 : _d.scopeTier) === 'universal';
    const checkGenericRelationship = (text, fieldName) => {
        if (!text)
            return;
        for (const pat of GENERIC_RELATIONSHIP_PHRASES) {
            if (pat.test(text)) {
                add(isUniversal ? 'error' : 'warn', 'generic-relationship-phrase', scenario.id, `${fieldName}: generic foreign reference detected${isUniversal ? ' in universal scenario — must be entirely domestic' : ' — use relationship tokens ({the_neighbor}, {the_adversary}, etc.) instead'}`);
                break;
            }
        }
    };
    ...
    const checkNewsroomReadability = (text, fieldName) => {
        if (!text)
            return;
        const jargonTerms = [
            {
        ...
        for (const term of jargonTerms) {
            if (term.pattern.test(text)) {
                add('error', 'newsroom-jargon', scenario.id, `${fieldName} uses jargon that reads unlike mainstream news copy. Replace it with plainer wording such as ${term.replacement}.`);
            }
        }
    };
    ...
    for (const opt of scenario.options) {
        ...
        for (const feedback of (_e = opt.advisorFeedback) !== null && _e !== void 0 ? _e : []) {
    ...
    if (anyTextUsesRulingPartyToken && ((_f = scenario.metadata) === null || _f === void 0 ? void 0 : _f.applicable_countries) && scenario.metadata.applicable_countries !== 'all') {
    ...
    if (scenario.metadata) {
        ...
        if (!((_g = meta.scopeKey) === null || _g === void 0 ? void 0 : _g.trim()))
        ...
        if (meta.scopeTier === 'cluster') {
            if (!((_h = meta.clusterId) === null || _h === void 0 ? void 0 : _h.trim())) {
    ...
    for (const opt of scenario.options) {
        ...
        if (!((_j = opt.text) === null || _j === void 0 ? void 0 : _j.trim())) {
        ...
        for (let i = 0; i < opt.effects.length; i++) {
            ...
            if (!Number.isFinite(eff.value)) {
                ...
                const cap = (_k = cfg.metricMagnitudeCaps[eff.targetMetricId]) !== null && _k !== void 0 ? _k : cfg.defaultCap;
        ...
        if (!((_l = opt.outcomeHeadline) === null || _l === void 0 ? void 0 : _l.trim()))
            add('error', 'missing-headline', oid, 'Missing headline', true);
        if (!((_m = opt.outcomeSummary) === null || _m === void 0 ? void 0 : _m.trim()))
        ...
        if (((_p = opt.outcomeSummary) === null || _p === void 0 ? void 0 : _p.trim()) && opt.outcomeSummary.trim().length < 250)
            add('warn', 'short-summary', oid, `outcomeSummary is ${opt.outcomeSummary.trim().length} chars (min 250)`, true);
        if ((_q = opt.outcomeContext) === null || _q === void 0 ? void 0 : _q.trim()) {
        ...
        if (!opt.advisorFeedback || !Array.isArray(opt.advisorFeedback) || opt.advisorFeedback.length === 0) {
            ...
            for (const fb of opt.advisorFeedback) {
                if (!fb.roleId || !fb.stance || !((_r = fb.feedback) === null || _r === void 0 ? void 0 : _r.trim()))
                    ...
                    const feedback = (_t = (_s = fb.feedback) === null || _s === void 0 ? void 0 : _s.trim()) !== null && _t !== void 0 ? _t : '';
    ...
    if (((_u = scenario.metadata) === null || _u === void 0 ? void 0 : _u.scopeTier) === 'universal' && relationshipTokensUsed.length > 0) {
        add('error', 'universal-relationship-token', scenario.id, `Universal scenarios cannot depend on country relationship tokens (${relationshipTokensUsed
            .map(formatRelationshipToken)
            .join(', ')}). Scope the scenario to compatible countries or rewrite it with universal framing.`, false);
    }
    const applicableCountries = Array.isArray((_v = scenario.metadata) === null || _v === void 0 ? void 0 : _v.applicable_countries)
        ? (_w = scenario.metadata) === null || _w === void 0 ? void 0 : _w.applicable_countries
        : [];
    const countriesById = (_x = cfg.countriesById) !== null && _x !== void 0 ? _x : {};
    if (relationshipTokensUsed.length > 0 && applicableCountries.length > 0) {
        for (const tokenName of relationshipTokensUsed) {
            ...
            if (unsupportedCountryIds.length > 0) {
                add('error', 'country-relationship-token-mismatch', scenario.id, `Scenario uses ${formatRelationshipToken(tokenName)} but applicable_countries include countries that cannot resolve it: ${unsupportedCountryIds.join(', ')}`, false);
            }
        }
    }
    ...
    if (!hasRequiredRelationshipToken && ((_y = scenario.metadata) === null || _y === void 0 ? void 0 : _y.scopeTier) !== 'universal') {
    ...
    if ((_z = scenario.title) === null || _z === void 0 ? void 0 : _z.trim()) {
    ...
    for (const opt of scenario.options) {
        ...
        for (const fb of ((_0 = opt.advisorFeedback) !== null && _0 !== void 0 ? _0 : [])) {
    ...
    for (const opt of scenario.options) {
        if ((_1 = opt.label) === null || _1 === void 0 ? void 0 : _1.trim()) {
    ...
    if (Array.isArray(scenario.conditions)) {
        scenario.conditions.forEach((cond, i) => {
            if (cond.metricId && !cfg.validMetricIds.has(cond.metricId)) {
                add('error', 'conditions-invalid-metric', scenario.id, `conditions[${i}] references unknown metricId "${cond.metricId}"`);
            }
            if (cond.min !== undefined && cond.max !== undefined && cond.min >= cond.max) {
                add('warn', 'conditions-unreachable-range', scenario.id, `conditions[${i}] has unreachable range (min ${cond.min} >= max ${cond.max})`);
            }
            if ((cond.max !== undefined && cond.max < 15) || (cond.min !== undefined && cond.min > 85)) {
                add('warn', 'conditions-too-restrictive', scenario.id, `conditions[${i}] threshold is extreme — scenario may rarely appear in gameplay`);
            }
        });
    }
    ...
    if (Array.isArray(scenario.relationship_conditions)) {
        scenario.relationship_conditions.forEach((rc, i) => {
            if (!rc.relationshipId || !VALID_RELATIONSHIP_IDS.has(rc.relationshipId)) {
                add('error', 'relationship-condition-invalid-role', scenario.id, `relationship_conditions[${i}] has unknown relationshipId "${rc.relationshipId}". Valid: ${[...VALID_RELATIONSHIP_IDS].join(', ')}`);
            }
            if ((rc.min !== undefined && (rc.min < -100 || rc.min > 100)) ||
                (rc.max !== undefined && (rc.max < -100 || rc.max > 100))) {
                add('error', 'relationship-condition-out-of-range', scenario.id, `relationship_conditions[${i}] min/max must be in −100..100 (relationship strength scale)`);
            }
            if (rc.min !== undefined && rc.max !== undefined && rc.min >= rc.max) {
                add('warn', 'relationship-condition-unreachable-range', scenario.id, `relationship_conditions[${i}] has unreachable range (min ${rc.min} >= max ${rc.max})`);
            }
        });
    }
    // Actor patterns with relational actors should declare relationship conditions
    const scenarioActorPattern = (_2 = scenario.metadata) === null || _2 === void 0 ? void 0 : _2.actorPattern;
    ...
    if (scenarioActorPattern && relationalPatterns.has(scenarioActorPattern)) {
        ...
        if (!hasRelCond) {
            add('warn', 'actor-pattern-no-relationship-condition', scenario.id, `Scenario uses actor pattern "${scenarioActorPattern}" but has no relationship_conditions — it may appear regardless of the actual relationship state`);
        }
    }
    // Option metric overlap — all 3 options hit the exact same set of metrics
    const optionMetricSets = scenario.options.map(opt => { var _a; return new Set(((_a = opt.effects) !== null && _a !== void 0 ? _a : []).map((e) => e.targetMetricId).filter(Boolean)); });
    if (optionMetricSets.length === 3) {
        const same = (a, b) => a.size === b.size && [...a].every(m => b.has(m));
        if (same(optionMetricSets[0], optionMetricSets[1]) && same(optionMetricSets[1], optionMetricSets[2])) {
            add('warn', 'option-metric-overlap', scenario.id, `All 3 options affect the same metric set (${[...optionMetricSets[0]].join(', ')}). Options should differentiate across domains.`);
        }
    }
    // Severity-effect mismatch — critical/extreme scenario with all tiny effects
    const severity = (_3 = scenario.metadata) === null || _3 === void 0 ? void 0 : _3.severity;
    if (severity === 'critical' || severity === 'extreme') {
        const allEffects = scenario.options.flatMap(opt => { var _a; return (_a = opt.effects) !== null && _a !== void 0 ? _a : []; });
        const hasSignificant = allEffects.some((e) => Number.isFinite(e.value) && Math.abs(e.value) >= 1.5);
        if (allEffects.length > 0 && !hasSignificant) {
            add('warn', 'severity-effect-mismatch', scenario.id, `Severity is "${severity}" but all ${allEffects.length} effects have |value| < 1.5. A ${severity} scenario needs at least one effect ≥ 1.5.`);
        }
    }
    // Advisor stance uniformity — same stance on every option for most advisors
    const stancesByRole = {};
    for (const opt of scenario.options) {
        for (const fb of ((_4 = opt.advisorFeedback) !== null && _4 !== void 0 ? _4 : [])) {
            if (!fb.roleId || !fb.stance)
                continue;
            if (!stancesByRole[fb.roleId])
                stancesByRole[fb.roleId] = new Set();
            stancesByRole[fb.roleId].add(fb.stance);
        }
    }
    const uniformCount = Object.values(stancesByRole).filter(s => s.size === 1).length;
    if (uniformCount > 8) {
        add('warn', 'advisor-stance-uniformity', scenario.id, `${uniformCount}/13 advisors have identical stance across all options. Options should create genuine advisor disagreement.`);
    }
    for (const opt of scenario.options) {
        if (!opt.policyImplications)
            continue;
        for (const impl of opt.policyImplications) {
            if (!types_1.VALID_SETTING_TARGETS.includes(impl.target)) {
                add('error', 'invalid-policy-target', scenario.id, `Option ${opt.id} has invalid policyImplication target: "${impl.target}"`);
            }
            if (typeof impl.delta !== 'number' || impl.delta < -15 || impl.delta > 15) {
                add('error', 'policy-delta-out-of-range', scenario.id, `Option ${opt.id} policyImplication delta ${impl.delta} out of range [-15, 15]`);
            }
        }
    }
    return issues;
...
function normalizeScenarioTextFields(scenario) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    let fixed = false;
    const fixes = [];
    const normalizeTokenCase = (text) => text === null || text === void 0 ? void 0 : text.replace(/\{([a-zA-Z_]+)\}/g, (_match, token) => `{${String(token).toLowerCase()}}`);
    const collapseRepeatedPhrase = (text) => text.replace(/\b([A-Za-z][A-Za-z'-]*(?:\s+[A-Za-z][A-Za-z'-]*){0,3})\s+\1\b/gi, '$1');
    const collapseRepeatedSentences = (text) => {
        var _a;
        const sentences = text.split(/(?<=[.!?])\s+/).filter(Boolean);
        const deduped = [];
        for (const sentence of sentences) {
            if (((_a = deduped[deduped.length - 1]) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === sentence.toLowerCase())
                continue;
    ...
    const normalizeText = (text) => {
        ...
        if (!text)
            return text;
        let result = (_a = normalizeTokenCase(text)) !== null && _a !== void 0 ? _a : text;
        result = (_b = repairTextIntegrity(result)) !== null && _b !== void 0 ? _b : result;
        result = collapseRepeatedPhrase(result);
        result = collapseRepeatedSentences(result);
        result = (_c = repairTextIntegrity(result)) !== null && _c !== void 0 ? _c : result;
        result = (_d = capitalizeFirstNarrativeLetter(result)) !== null && _d !== void 0 ? _d : result;
    ...
    scenario.title = (_a = apply(scenario.title, 'normalized title')) !== null && _a !== void 0 ? _a : scenario.title;
    scenario.description = (_b = apply(scenario.description, 'normalized description')) !== null && _b !== void 0 ? _b : scenario.description;
    for (const option of scenario.options) {
        option.text = (_c = apply(option.text, `${option.id}: normalized text`)) !== null && _c !== void 0 ? _c : option.text;
        if (typeof option.outcomeHeadline === 'string') {
            option.outcomeHeadline = (_d = apply(option.outcomeHeadline, `${option.id}: normalized outcomeHeadline`)) !== null && _d !== void 0 ? _d : option.outcomeHeadline;
        }
        if (typeof option.outcomeSummary === 'string') {
            option.outcomeSummary = (_e = apply(option.outcomeSummary, `${option.id}: normalized outcomeSummary`)) !== null && _e !== void 0 ? _e : option.outcomeSummary;
        }
        if (typeof option.outcomeContext === 'string') {
            option.outcomeContext = (_f = apply(option.outcomeContext, `${option.id}: normalized outcomeContext`)) !== null && _f !== void 0 ? _f : option.outcomeContext;
        }
        for (const feedback of (_g = option.advisorFeedback) !== null && _g !== void 0 ? _g : []) {
            feedback.feedback = (_h = apply(feedback.feedback, `${option.id}: normalized advisor feedback`)) !== null && _h !== void 0 ? _h : feedback.feedback;
        }
    }
    return { fixed, fixes };
}
function deterministicFix(scenario) {
    ...
    const fixWhitespace = (text) => {
        ...
        if (!text)
            return text;
        return ((_a = repairTextIntegrity(text)) !== null && _a !== void 0 ? _a : text)
            .replace(/  +/g, ' ')
            .replace(/\. \./g, '.')
            .replace(/(?<!\.)\.{2}(?!\.)/g, '.')
            .replace(/, ,/g, ',')
