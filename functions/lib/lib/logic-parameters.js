...
exports.NARRATIVE_TO_METRIC_ALIGNMENT = exports.LOGIC_PARAMETERS = exports.ALL_METRIC_IDS = exports.VALID_METRICS = exports.isValidBundleId = exports.ALL_BUNDLE_IDS = exports.BUNDLE_IDS = exports.METRIC_IDS = void 0;
...
2. **Template Tokens**: Use these placeholders in descriptions/outcomes:
   - Executive: {${token_registry_1.TOKEN_CATEGORIES.executive.join('}, {')}}
   - Legislative: {${token_registry_1.TOKEN_CATEGORIES.legislative.join('}, {')}}
   - Judicial: {${token_registry_1.TOKEN_CATEGORIES.judicial.join('}, {')}}
   - Ministers: {${token_registry_1.TOKEN_CATEGORIES.ministers.join('}, {')}}
   - Security: {${token_registry_1.TOKEN_CATEGORIES.security.join('}, {')}}
   - Military: {${token_registry_1.TOKEN_CATEGORIES.military.join('}, {')}}
   - Economic: {${token_registry_1.TOKEN_CATEGORIES.economic.join('}, {')}}
   - Local: {${token_registry_1.TOKEN_CATEGORIES.local.join('}, {')}}
   - Civil: {${token_registry_1.TOKEN_CATEGORIES.civil.join('}, {')}}
   - Media: {${token_registry_1.TOKEN_CATEGORIES.media.join('}, {')}}
   - Country/relationship — subject, object, or prepositional position → ALWAYS use the {the_*} article form (auto-adds "the" where required, e.g. "the United States" but not "Germany"):
     {${token_registry_1.TOKEN_CATEGORIES.article_forms.filter(t => ['the_adversary', 'the_border_rival', 'the_regional_rival', 'the_ally', 'the_neutral', 'the_rival', 'the_partner', 'the_trade_partner', 'the_neighbor', 'the_player_country', 'the_nation', 'the_regional_bloc'].includes(t)).join('}, {')}}
   - Possessive constructions only → bare form immediately followed by 's is valid: {${token_registry_1.TOKEN_CATEGORIES.relationships.join('}, {')}}
   ...
   - Article forms ({the_*}) for roles and institutions — use when a role title is a sentence subject or follows a preposition:
     {${token_registry_1.TOKEN_CATEGORIES.article_forms.join('}, {')}}
...
${token_registry_1.CONCEPT_TO_TOKEN_MAP.map(({ concept, token }) => `   - ${concept} → ${token}`).join('\n')}
